package edu.mtu.utilities;

/**
 * This class acts a wrapper for useful constants that may be used.
 */
public final class Constants {
	/**
	 * Size of one acre of land in square meters.
	 */
	public final static double acreInSquareMeters = 4046.86;

	/**
	 * The height of a standard diameter at breast height measurement, in meters.
	 */
	public final static double DbhTakenAt = 1.37;
	
	/**
	 * The number of centimeters in an inch.
	 */
	public final static double InchToCentimeter = 2.54;
	
	/**
	 * Value to divide by to convert from kilograms to gigatons.
	 */
	public final static long KilogramToMetricTon = 1000;
	
	/**
	 * Value to multiply by to go from meters to feet;
	 */
	public final static double MeterToFoot = 3.281;
	
	/**
	 * Value to divide by to convert form metric tons to gigatons.
	 */
	public final static long MetricTonToGigaTon = 1000000000;
	
	/**
	 * Value to divide by to convert from sq.m to ha.
	 */
	public final static long SquareMetersToHectares = 10000;
	
	/**
	 * Value to divide by to convert from sq.m. to sq.km.
	 */
	public final static long SquareMetersToSquareKilometers = 1000000;
}
